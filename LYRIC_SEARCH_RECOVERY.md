...
歌手/歌名编码、Code 校验、`ci` 和响应解析均交由原 DLL 完成，不在主程序
另写一套 HTTP 客户端。原插件仍保留它自身的 HTTP/WinINet 限制。

重建主配置仍为 `TTPlayerRebuild.xml`：`Lyric/AddInIndex` 仅选择 provider，
