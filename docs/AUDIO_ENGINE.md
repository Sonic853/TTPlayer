...
按用户要求，WaveOut 另增加单句柄串行淡化：旧曲完成停止淡出后再打开新曲并淡入，
支持连续导航、更换待播目标和停止取消，避免两个输出句柄争用设备音量。
原版虚表、选项对应关系和恢复边界见
...
恢复 Winamp 输入/输出回调桥接、增强采样率判定、环形 PCM、暂停/定位/停止，
并保留普通 MPEG 回退、metadata 和现有处理/输出链。不依赖 Winamp.exe，
也不检查 DLL 文件版本号。另修正异步预填充导致暂停 waveOut 定位推进的问题。
原版地址、恢复范围、明确差异及真实/模拟测试边界见
[MP3PRO_RECOVERY.md](MP3PRO_RECOVERY.md)。
