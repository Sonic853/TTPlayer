...
播放列表背景 `#1d3840` 等覆盖值；外置副本尚无自己的配置。
皮肤包里的 `Lyric.xml`、`Playlist.xml` 默认背景则分别是
`#31475b`、`#4b6782`。因此已有配置不同，本身就会造成外观差异。

后续对原版初始化函数的核对确认，`#18333c`、`#1d3840` 也是原版通用初始化值，
不能仅凭这些颜色认定用户手动改过配色。详见[原版默认配色分析](ORIGINAL_DEFAULT_PALETTE.md)。
...
普通启动的播放列表／歌词字体和颜色则先读取实际包内默认值，再逐字段叠加主配置和
当前皮肤配置中的明确设置，避免首次运行或稀疏配置使用通用灰色主题。详见
[无配置首次启动修复](SKIN_FRESH_STARTUP.md)。
