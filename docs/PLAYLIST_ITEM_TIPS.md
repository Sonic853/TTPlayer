...
## waskin 曲目提示（2026-09-23）

此前 DLL 的 `TipText` 排除了歌曲行，只有按钮提示。现通过现有 `TtpSkinHost.tip`
回调新增 `TTP_SKIN_TRACK_TIP` 查询，传入 DLL 当前命中的可见曲目索引。宿主与原生
列表共用 `PlaylistItemTipText`，沿用下文的资源模板、空字段省略、网络地址及文件
缺失提示，并遵守 `ItemTips` 和 `ReadInfoMode`。媒体库模式沿用 `VisiblePlaylistTrack`
的查询结果映射。

DLL 仅为鼠标所在歌曲注册单行矩形，使用 400 像素宽度和系统默认悬停延时；换行、
滚动、折叠、调整大小、拖动、移出和切回原生皮肤时清除旧提示。空白区域没有曲目提示。
提示内容在实际悬停通知中请求，支持按需分配长文本，不复用按钮的 512 字符缓冲区。
未读取的本地曲目提交到已有后台信息队列；显式悬停请求不按隐藏的原生列表范围裁剪。
显示中的提示每 500 ms 检查一次内容变化，信息更新后刷新，关闭 `ItemTips` 后隐藏。

ABI 结构大小及已有指令值不变。新查询返回含 NUL 的 WCHAR 数量，`nullptr/0` 查询
所需长度；返回 0 表示不显示。旧宿主不认识该指令时不显示歌曲提示，按钮仍正常工作。
该功能需要同时更新宿主 EXE 与 `AddIn/ttp_waskin.dll`。

本地回归位于 `tests/waskin`，包括 DLL 命中／长文本／旧接口、双宿主原生文本一致性、
提示开关、后台请求去重、滚动和实际悬停弹窗动态更新。测试不进入发行包或 Actions。
